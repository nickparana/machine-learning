...
class NeuronalNetwork {
    ...
    train(data) {
        ...
        for (let i = 0; i < this.epoch; i++) {
            let d = 0;
            for (; d < maxData; d++) {
                let input = math.matrix(this.xMean(data[d].input), 'dense');
                let output = math.matrix(data[d].target, 'dense');
                this.forward(input);
                let error = math.subtract(this.Y, output);
                let errorT = math.matrix([error._data]);
                let dLw2 = math.multiply(math.matrix(this.H._data.map(e => [e])), errorT);
    ...
    guess(input) {
        this.forward(math.matrix(this.xMean(input), 'dense'));
